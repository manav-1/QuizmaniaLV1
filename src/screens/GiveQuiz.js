...
export default function GiveQuiz({
  route: { params: { quizId, quizImgUri, quizName, questions = [] } = {} } = {},
  navigation,
}) {
  ...
  const [selectedQstnResponseOptionIdx, setSelectedQstnResponseOptionIdx] =
    useState(null);
      ...
      return (
        <ScrollView style={styles.scroll}>
          <View style={styles.qstnContainer}>
            <Text style={styles.qstnCount}>
              {activeQstnIdx + 1 + " of " + totalQstnsCount}
            </Text>
          ...
          {options.map((item, idx) => {
            ...
            if (
              (selectedOptionId == optionId ||
                selectedQstnResponseOptionIdx == optionId) &&
              isAns
            ) {
  ...
  async function handleSubmitBtnClick() {
    ...
    if (loggedUserId && quizId) {
      ...
      usersDbRef.child(loggedUserId + "/quizResponses/" + quizId).set(
        {
          quizId: quizId,
    ...
    <View style={styles.root}>
      {isLoading ? (
        ...
        <>
          ...
          <Image
            source={quizImgUri || require("../../assets/quiz.jpg")}
            style={styles.image}
          />
